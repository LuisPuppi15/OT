...
class ImageService extends EventProvider implements ImageServiceInterface {
	...
	public function getImage($relativePath, $filter) {
		...
		$eventManager->trigger(__FUNCTION__, $this, array('relativePath' => $relativePath, 'filter' => $filter));
		...
		$args = array('relativePath' => $relativePath, 'filter' => $filter, 'filteredImage' => $filteredImage, 'format' => $format);
