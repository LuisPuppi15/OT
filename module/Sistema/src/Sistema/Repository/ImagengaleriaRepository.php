 ...
 */
class ImagengaleriaRepository extends EntityRepository {
	...
	public function buscar($campos, $paginacion) {
		...
		$qb->select('ig')
		   ...
		   ->leftJoin('ig.galeria', 'g')
		   ->where('ig.nimagalestado != :nimagalestado')
		   ->orderBy('ig.cimagaljerarquia', 'ASC')
		   ->setParameter('nimagalestado', 0);
	...
	public function buscarUltimoOrden() {
		$qb = $this->_em->createQueryBuilder();
		$qb->select('ig.cimagaljerarquia')
		   ->from('Sistema\Entity\Imagengaleria', 'ig')
		   ->orderBy('ig.cimagaljerarquia', 'DESC')
		   ->setFirstResult(0)
		   ->setMaxResults(1);
	...
	public function buscarMenoresParaOrdenar($cimagaljerarquiaMay, $cimagaljerarquiaMin) {
		$qb = $this->_em->createQueryBuilder();
		$qb->select('ig')
		   ->from('Sistema\Entity\Imagengaleria', 'ig')
		   ->where('ig.cimagaljerarquia > :cimagaljerarquiaMin')
		   ->andWhere('ig.cimagaljerarquia <= :cimagaljerarquiaMay')
		   ->setParameter('cimagaljerarquiaMay', $cimagaljerarquiaMay)
		   ->setParameter('cimagaljerarquiaMin', $cimagaljerarquiaMin);

		$query = $qb->getQuery();
		return $query->getResult();
	}

	public function buscarMayoresParaOrdenar($cimagaljerarquiaMin, $cimagaljerarquiaMay) {
		$qb = $this->_em->createQueryBuilder();
		$qb->select('ig')
		   ->from('Sistema\Entity\Imagengaleria', 'ig')
		   ->where('ig.cimagaljerarquia >= :cimagaljerarquiaMin')
