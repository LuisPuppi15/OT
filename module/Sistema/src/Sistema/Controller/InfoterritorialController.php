...
use Sistema\Form\InfoterritorialForm;
use Zend\Mvc\Controller\AbstractActionController;
use Zend\Session\Container;
use Zend\View\Model\JsonModel;
use Zend\View\Model\ViewModel;

class InfoterritorialController extends AbstractActionController {
	private $usuarioLogueado;
	private $nctrcodigo = 61;
	private $rutaRaiz = 'sistema/infoterritorial';
	private $items = 1;
	private $em;
	private $emNamespace = 'Sistema\Entity\Infoterritorial';
	private $repo;
	...
	public function inicioAction() {
		...
		$campos = array();
		$infoterritorials = $this->repo->buscar($campos, $paginacion);
		...
		$view = new ViewModel();
		$view->setVariable('infoterritorials', $infoterritorials)
	...
	public function filtrarAction() {
		...
		$viewLista->setTemplate('sistema/infoterritorial/lista.phtml')
		          ->setVariable('infoterritorials', $infoterritorials)
		          ->setTerminal(true);
	...
	public function editarAction() {
		...
		$infoterritorial = $this->repo->find($id);

		$form = new InfoterritorialForm($this->em);
		$form->bind($infoterritorial);
		...
		if ($request->isPost()) {
			...
			if ($form->isValid()) {
				$this->em->persist($infoterritorial);
				$this->em->flush();
