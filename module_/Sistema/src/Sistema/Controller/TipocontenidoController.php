...
use Sistema\Entity\Tipocontenido;
use Sistema\Form\TipocontenidoForm;
...
class TipocontenidoController extends AbstractActionController {
	private $usuarioLogueado;
	private $nctrcodigo = 73;
	private $rutaRaiz = 'sistema/tipocontenido';
	...
	private $items;
	private $em;
	private $emNamespace = 'Sistema\Entity\Tipocontenido';
	private $repo;
	...
	public function inicioAction() {
		...
		$campos = array();
		$tipocontenidos = $this->repo->buscar($campos, $paginacion);
		...
		$view = new ViewModel();
		$view->setVariable('tipocontenidos', $tipocontenidos)
	...
	public function filtrarAction() {
		...
		$viewLista->setTemplate('sistema/tipocontenido/lista.phtml')
		          ->setVariable('tipocontenidos', $tipocontenidos)
		          ->setTerminal(true);
	...
	public function agregarAction() {
		...
		$form = new TipocontenidoForm($this->em);
		$form->bind($tipocontenido);
		...
		if ($request->isPost()) {
			...
			if ($form->isValid()) {
				$tipocontenido->setNtipconestado(1);

				$this->em->persist($tipocontenido);
				$this->em->flush();
	...
	public function editarAction() {
		...
		$tipocontenido = $this->repo->find($id);
	...
	public function eliminarAction() {
		...
		$request = $this->getRequest();
		if ($request->isPost()) {
			$tipocontenido->setNtipconestado(0);
			$this->em->persist($tipocontenido);
			$this->em->flush();
		...
		$view = new ViewModel();
		$view->setVariable('tipocontenido', $tipocontenido)
		     ->setVariable('form', $form)
		     ->setTerminal(true);
