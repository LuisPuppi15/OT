...
class BoletinRepository extends EntityRepository {
	...
	public function buscar($campos, $paginacion) {
		...
		$qb->select('b')
		   ...
		   ->where('b.nbolestado != :nbolestado')
		   ->orderBy('b.cboljerarquia', 'ASC')
		   ->setParameter('nbolestado', 0);
	...
	public function buscarUltimoOrden() {
		$qb = $this->_em->createQueryBuilder();
		$qb->select('b.cboljerarquia')
		   ->from('Sistema\Entity\Boletin', 'b')
		   ->orderBy('b.cboljerarquia', 'DESC')
		   ->setFirstResult(0)
		   ->setMaxResults(1);
	...
	public function buscarMenoresParaOrdenar($cboljerarquiaMay, $cboljerarquiaMin) {
		$qb = $this->_em->createQueryBuilder();
		$qb->select('b')
		   ->from('Sistema\Entity\Boletin', 'b')
		   ->where('b.cboljerarquia > :cboljerarquiaMin')
		   ->andWhere('b.cboljerarquia <= :cboljerarquiaMay')
		   ->setParameter('cboljerarquiaMay', $cboljerarquiaMay)
		   ->setParameter('cboljerarquiaMin', $cboljerarquiaMin);

		$query = $qb->getQuery();
		return $query->getResult();
	}

	public function buscarMayoresParaOrdenar($cboljerarquiaMin, $cboljerarquiaMay) {
		$qb = $this->_em->createQueryBuilder();
		$qb->select('b')
		   ->from('Sistema\Entity\Boletin', 'b')
		   ->where('b.cboljerarquia >= :cboljerarquiaMin')
