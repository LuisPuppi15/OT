...
(function($) {
    ...
    FileInput.prototype = {
        ...
        listen: function() {
            ...
            self.$container.find('.kv-fileinput-upload').on('click', function(e) {
                ...
                self.uploadCount = 0;
                self.uploadPercent = 0;
                var i, len = self.filestack.length,
                    template = self.getLayoutTemplate('progress');
                setTimeout(function() {
                    self.lock();
                    self.setProgress(0);
        ...
        renderInitFileFooter: function(i) {
            var self = this,
                hasConfig = self.initialPreviewConfig.length > 0,
        ...
        clear: function() {
            var self = this,
                e = arguments.length && arguments[0];
                        ...
                        if (typeof data.error === 'undefined' || isEmpty(data.error)) {
                            ...
                            self.$preview.find('.file-preview-frame').each(function() {
                                ...
                                if ($.inArray(key, keys)) {
                                    ...
                                    var $upload = $thumb.find('.kv-file-upload'),
        ...
        previewDefault: function(file, previewId) {
            var self = this,
                data = vUrl.createObjectURL(file),
                $obj = $('#' + previewId),
        ...
        readFiles: function(files) {
            ...
            function readFile(i) {
                ...
                var node = ctr + i,
                    ...
                    checkFile,
                    previewData = vUrl.createObjectURL(file),
                    fileCount = 0,
                    j, msg, typ, chk,
    ...
    $.fn.fileinput.defaults = {
        ...
        msgProgress: 'Cargando archivo {index} de {files} - {name} - {percent}% completed.',
